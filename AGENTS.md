...
- **AVA (Asterisk AI Voice Agent)** at `/opt/ava` (compose project `asterisk-ai-voice-agent`): `ai_engine` (health/metrics loopback-only `:15000`, AudioSocket `127.0.0.1:8090`) and `admin_ui` (`:3003`, publicly reachable — restrict by firewall/VPN). `local_ai_server` is stopped on purpose (cloud providers only, RAM budget). Ava one-time password file was deleted after first login.
